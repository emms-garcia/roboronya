...
class Roboronya(object):
        ...
        for command in commands:
            ...
            try:
                return self.run_command(
                    command['name'], command['args'], **kwargs
                )
    ...
    @asyncio.coroutine
    def run_command(self, name, cmd_args, **kwargs):
        logger.info(
        ...
        cmd_args[0] = self
        command_func = getattr(Commands, name)
        return command_func(*cmd_args, **kwargs)
    ...
    def connect(self):
        ...
        if not hasattr(self, '_cookies'):
            self._cookies = hangups.auth.get_auth_stdin(
                REFRESH_TOKEN_PATH
            )
            if not self._cookies:
                logger.info('Invalid login.')
                sys.exit(0)
        self._hangups = hangups.Client(self._cookies)
